...
import { addCategory, addTodo, deleteTodo, getCategories, getCategory, getTodo, getTodos, signIn, signUp, updateTodo, } from "./requests.js";
...
app.get("/get-todos", async (req, res) => {
    ...
    try {
        const response = await getTodos(categoryID);
        if (response.status) {
            res.send(response.data).status(response.status);
        }
        else {
