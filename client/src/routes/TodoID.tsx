...
const TodoID = () => {
	...
	useEffect(() => {
		;(async () => {
			try {
				const res = await client.get("/get-todo", { params: { id: todoID } })
				const data: ITodo = res.data.items[0]
				setTodoName(data.title)
				setTodoDetails(data.details)
				setTodoDue(data.deadline)
	...
	async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
		...
		setIsLoading(true)
		try {
			const res = await client.put("/update-todo", {
				title: todoName,
				details: todoDetails,
				due: todoDue,
				categoryID: id,
							...
							<Dialog.Panel className='w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all'>
								...
								>
									...
									<div>
										...
										/>
										<label htmlFor='status'>Mark as complete</label>
									</div>
